...
3. **Preservação de Áudio Nativo**: Extração do melhor stream sem perdas de transcodificação (`.m4a`/`.opus`), com conversão opcional para MP3 320 kbps sob demanda (`--format mp3`).
4. **Isolamento de Staging & Taxonomia Canônica**: Ingestão particionada por sessões com manifestos `session.json`, promoção não-destrutiva e estrutura física em `Artists/<Artist>/<Album>`, `Various Artists/<Album>` e `Collections/<Name>`.
5. **Governança Multiplataforma da Biblioteca**: Resolução canônica em `~/Music/MusicMatch/` (Windows, macOS, Linux), verificação atômica de escrita (`.musicmatch_probe`) e bloqueio de caminhos críticos do SO.
